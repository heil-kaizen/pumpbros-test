...
export class Game {
  ...
  updateSelect() {
    ...
    if (input.pointerPressed) {
      ...
      // Calculate coordinates from renderSelect
      ...
      for (let i = 0; i < CHARACTERS.length; i++) {
        ...
        if (px >= x && px <= x + cw && py >= y && py <= y + ch) {
          // find an unlocked cursor to set
          const unlockedCur = this.cursors.find(c => !c.locked);
          if (unlockedCur) {
            unlockedCur.idx = i;
            unlockedCur.locked = true;
            sfx.start();
          } else {
             // Maybe user wants to unlock their cursor by clicking a character while already locked?
             // Since clicking is fast, let's just ignore or allow unlocking if they click their existing?
             // Simple fallback: do nothing if all locked.
          }
        }
      }
    }
  ...
  updatePause() {
    ...
    if (input.pointerPressed) {
      ...
      for (let i = 0; i < 3; i++) {
        const by = cy - 40 + i * 45;
        if (px >= cx - 100 && px <= cx + 100 && py >= by && py <= by + 32) {
          if (this.pauseIndex !== i) {
             this.pauseIndex = i;
             sfx.select();
          } else {
             input.pressed.add('Enter'); // trigger below
          }
        }
      }
    }
